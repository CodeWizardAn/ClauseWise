...
export interface Protected {
  /** The text with protected values replaced by {0}, {1}, ... */
  template: string;
  /** The removed values, in slot order. */
  values: string[];
}
...
export function protectText(text: string): Protected {
  const values: string[] = [];
...
export function restoreText(template: string, values: string[]): string {
