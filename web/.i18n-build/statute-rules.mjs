 ...
 */
import { statutesByIds } from "./statutes.mjs";
/**
...
export function extractTermMonths(text) {
    ...
    for (const [word, value] of Object.entries(NUMBER_WORDS)) {
        if (new RegExp(`\\b${word}\\s+months?\\b`).test(lowered))
            return value;
    }
    ...
    for (const [word, value] of Object.entries(NUMBER_WORDS)) {
        if (new RegExp(`\\b${word}\\s+years?\\b`).test(lowered))
            return value * 12;
    }
    return null;
}
function missing(omission, id) {
    return omission.missing.some((item) => item.id === id);
}
function flag(id, title, why, observed, severity, confidence, statuteIds) {
    return { id, title, why, observed, severity, confidence, statutes: statutesByIds(statuteIds) };
}
export function evaluateStatuteRules(context) {
    ...
    if (type === "rental") {
        ...
        if (months !== null && months > REGISTRATION_THRESHOLD_MONTHS && missing(omission, "registration")) {
            flags.push(flag("unregistered-registrable-lease", "The agreement runs beyond 11 months but says nothing about registration", "Under the Registration Act, 1908, a lease of immovable property beyond a short term is meant to be registered. " +
                "An agreement that should have been registered but was not carries much less weight as evidence, so if the owner " +
                "later disputes your tenancy you may struggle to prove it. Ask for the agreement to be registered.", `Term read as ${months} months, and no registration clause was found.`, "serious", "established", ["registration-act-1908", "transfer-of-property-1882"]));
        }
        ...
        if (figures.deposit && figures.rent && figures.rent.amount > 0) {
            ...
            if (multiple > DEPOSIT_MONTHS_REFERENCE) {
                flags.push(flag("deposit-above-reference", "The security deposit is large relative to the rent", `The Model Tenancy Act, 2021 proposes a cap of ${DEPOSIT_MONTHS_REFERENCE} months' rent as a residential security deposit, ` +
                    ...
                    "your property is, not a conclusion that the deposit breaches them.", `Deposit is about ${multiple.toFixed(1)} months' rent.`, "caution", "varies-by-state", ["model-tenancy-2021", "state-rent-control"]));
            }
        }
        // 3. A lock-in that binds only the tenant.
        const hasLockIn = /lock[\s-]?in/i.test(text);
        const oneSidedExit = /(?:licensor|lessor|landlord|owner)[^.]{0,140}(?:terminate|revoke)[^.]{0,140}without assigning any reason/i.test(text) ||
            /may terminate[^.]{0,120}without assigning any reason/i.test(text);
        if (hasLockIn && oneSidedExit) {
            flags.push(flag("one-sided-lock-in", "You are locked in while the other side can leave at will", "A lock-in that binds only you, next to a clause letting the owner end the agreement without giving a reason, " +
                "is a one-sided bargain. Under the Indian Contract Act, 1872 a term this unbalanced can be argued against, and " +
                "it is worth asking for the lock-in to apply to both sides or for the notice periods to match.", "The document has a lock-in period and also lets the owner terminate without assigning a reason.", "serious", "established", ["contract-act-1872", "model-tenancy-2021"]));
        }
    }
    if (type === "loan") {
        ...
        if (penalty) {
            ...
            if ((perMonth && penalty.percent >= PENAL_MONTHLY_PERCENT) ||
                (!perMonth && penalty.percent >= PENAL_ANNUAL_PERCENT)) {
                flags.push(flag("penal-interest-high", "The penalty rate is steep", "The RBI Fair Practices Code for Lenders expects penal charges to be reasonable and proportionate to the default, " +
                    "and not used as a revenue source. A rate at this level turns one missed payment into a fast-growing debt. " +
                    "If your lender is RBI-regulated, ask for their published penal charges policy and check this against it.", perMonth
                    ? `Penalty read as ${penalty.percent}% per month — roughly ${annualised}% a year on top of the interest.`
                    : `Penalty read as ${penalty.percent}% a year on top of the interest.`, "serious", "established", ["rbi-fair-practices", "contract-act-1872"]));
            }
        }
        // 5. No prepayment terms at all.
        if (missing(omission, "prepayment-terms")) {
            flags.push(flag("missing-prepayment-terms", "No prepayment terms are stated", "Without stated prepayment terms you cannot tell what it costs to close the loan early and stop paying interest. " +
                "The RBI Fair Practices Code for Lenders requires regulated lenders to disclose terms clearly up front. Ask for the " +
                "prepayment and foreclosure charges in writing before signing.", "No prepayment or foreclosure clause was found.", "caution", "established", ["rbi-fair-practices", "contract-act-1872"]));
        }
        // 6. No grievance route.
        if (missing(omission, "grievance-redressal")) {
            flags.push(flag("missing-grievance-redressal", "No grievance redressal process is named", "The RBI Fair Practices Code for Lenders requires regulated lenders to run a grievance redressal mechanism and tell " +
                "borrowers how to reach it. With no route named here, a billing or recovery dispute has nowhere to go inside the " +
                "lender. Ask who the nodal officer is.", "No grievance, nodal officer or ombudsman clause was found.", "caution", "established", ["rbi-fair-practices"]));
        }
    }
    ...
    if (/reserves the right to (?:revise|change|modify|amend|alter)/i.test(text)) {
        flags.push(flag("unilateral-variation", "One side can change the terms after you sign", "A contract is an agreement between two parties, so a term letting one of them rewrite it alone is open to challenge " +
            "under the Indian Contract Act, 1872 — particularly where you had no ability to negotiate. At minimum, ask for advance " +
            "notice and a right to exit without penalty if the terms change.", "The document lets one party revise terms after signing.", "caution", "established", ["contract-act-1872"]));
    }
    return flags;
}
